...
def saveImage(array, path, bit_depth=8, rescale=True):
    ...
    # Save all frames
    if len(array.shape)==3 and array.shape[0] > 0:
...
# ------------
# Save a stack
def saveStack(array, path, bit_depth=8, rescale=True):
...
def saveVideo(array, file_name, fps=25, video_codec='libx264'):
