...
def _block_average(image_array, window, include_partial=False):
...
def _running_average(image_array, window, include_partial=False):
...
def averageStack(image_array, window, average_type='block', include_partial=False):

    # Do the block averaging
    if average_type == 'block':
        new_array = _block_average(image_array, window, include_partial=include_partial)

    # Do the running average
    elif average_type == 'running':
        new_array = _running_average(image_array, window, include_partial=include_partial)
