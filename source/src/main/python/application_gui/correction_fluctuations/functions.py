...
class fluctuationCorrectionFunctions(object):
    ...
    # -------------------------------------
    # Initialise the content of the display
    def initialiseDisplay(self):

        # Set the combo boxes settings
        _index_correction = self.correctionSelectionBox.findText(self.parent.config.intensity_correction_type.capitalize(), qtc.Qt.MatchFixedString)
        if _index_correction >= 0:
             self.correctionSelectionBox.setCurrentIndex(_index_correction)
    ...
    def correctFluctuations(self):
        ...
        if do_correction:
            ...
            self.image_array = intensityCorrection(self.image_array, correction=correction_type)
