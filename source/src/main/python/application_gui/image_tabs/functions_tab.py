...
class mainTabFunctions:
    ...
    def tabChanged(self, index):
        ...
        # Check if the trajectory controller dock is open
        if "tracking" in self.parent.docks.keys():
            if self.parent.docks["tracking"] is not None:

                # Get the trajectory list loaded in the combobox
                if self.displayedTabs[index].image_class.trajectory is not None:
                    self.displayedTabs[index].refreshPathList()

                # Load an empty list in the combobox
                else:
                    self.displayedTabs[index].refreshPathList(path_list=['None'])
