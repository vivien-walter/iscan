...
def loadImageFile(parent, path):
    ...
    msg.setInformativeText("""Single image stack file opening have not been implemented yet.
...
def checkItemType(parent, item):
    ...
    if len(item) != 1:
        ...
        msg.setInformativeText("""The software can only open one file or folder at the time.""")
        msg.setWindowTitle("ERROR")
        msg.setStandardButtons(qtw.QMessageBox.Ok)
        returnValue = msg.exec_()
        return 0
...
def saveTable(parent, allData):
    ...
    columnNames = ['Name', 'Contrast', 'ContrastErr', 'Noise', 'NoiseErr', 'SNR', 'SNRErr', 'Frame', 'X', 'Y', 'Angle', 'Length', 'Fit', 'Amplitude', 'AErr', 'Center', 'CErr', 'Width', 'WErr', 'Offset', 'OErr']
    headerText = ""
...
def saveProfiles(parent, allData):
    ...
    msg.setInformativeText("""All the profiles and their fit have been saved in the files.""")
...
def saveAllStats(parent, dataDict):
    ...
    for param in parameters:
        currentArray = [ param, str(np.mean(dataDict[param])), str(np.std(dataDict[param],ddof=1)) ]
        for param2 in parameters:
            currentArray.append( str(np.corrcoef(dataDict[param], dataDict[param2], ddof=1)[0,1]) )
...
def saveStats(parent, dataArray, parameterNames, numberColumns):
    ...
    if numberColumns == 2:
        ...
        headerText = 'Mean '+valueNames[0]+':' + str(np.mean(dataArray[:,0])) + '\n'
        headerText += 'StDev '+valueNames[0]+':' + str(np.std(dataArray[:,0],ddof=1)) + '\n'
        headerText += 'Mean '+valueNames[1]+':' + str(np.mean(dataArray[:,1])) + '\n'
        headerText += 'StDev '+valueNames[1]+':' + str(np.std(dataArray[:,1],ddof=1)) + '\n'

        # Get the linear correlation estimator
        headerText += "Spearman's Rho:" + str( np.corrcoef(dataArray[:,0], dataArray[:,1], ddof=1) ) + '\n'
