...
def contrastCorrection(imageArray, maxValue=1., minPV=None, maxPV=None):
...
def positionAdjustment(imageArray, cursorPosition, areaSize=30):
    ...
    subRow, subColumn = (np.unravel_index(np.argmax(smoothedArray, axis=None), smoothedArray.shape))
    subRow += row - areaSize
    ...
    return int(round(fittedColumn,0)) + subColumn - areaSize, int(round(fittedRow,0)) + subRow - areaSize

#-------------------------------------
# Simple correction of the image stack
def imageCorrection(array, averageType='median', correctionType='division', bitDepth=16, signedBits=True):
    ...
    if averageType == 'median':
        averageArray = np.reshape( np.median(unsignedArray, axis=0), (1, array.shape[1], array.shape[2]))
    elif averageType == 'mean':
        averageArray = np.reshape( np.mean(unsignedArray, axis=0), (1, array.shape[1], array.shape[2]))
