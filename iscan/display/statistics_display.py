...
class profilesAnalysisPanel(qtw.QMainWindow):
    def __init__(self, parent):
        ...
        if profileNumbers >= 2:
            ...
            msg.setInformativeText(
                """At least two profiles need to be saved in the memory to perform statistics."""
            )
    ...
    def createSelectionDisplay(self, parentWidget):
        ...
        self.xCenterInput.setText(
            str(self.parent.imageTabsImage[tabIndex].currentArray.shape[0] / 2)
        )
        ...
        self.yCenterInput.setText(
            str(self.parent.imageTabsImage[tabIndex].currentArray.shape[1] / 2)
        )
    ...
    def extractData(self, allData):
        ...
        self.dataDict = {
            ...
            "x": np.copy(self.dataArray[:, 8]).astype(float),
            "y": np.copy(self.dataArray[:, 9]).astype(float),
            "amplitude": np.copy(self.dataArray[:, 13]).astype(float),
            "amplitudeErr": np.copy(self.dataArray[:, 14]).astype(float),
            "center": np.copy(self.dataArray[:, 15]).astype(float),
            "centerErr": np.copy(self.dataArray[:, 16]).astype(float),
            "width": np.copy(self.dataArray[:, 17]).astype(float),
            "widthErr": np.copy(self.dataArray[:, 18]).astype(float),
            "offset": np.copy(self.dataArray[:, 19]).astype(float),
            "offsetErr": np.copy(self.dataArray[:, 20]).astype(float),
        }
