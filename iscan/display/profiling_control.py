...
class profilingControlPanel(qtw.QDockWidget):
    ...
    def createProfileDisplay(self, parentWidget):
        ...
        self.profileAxis.tick_params(axis='x',which='both',bottom=False,top=False,labelbottom=False)
        self.profileAxis.tick_params(axis='y',which='both',left=False,right=False,labelleft=False)
    ...
    def createFitFunctions(self, parentWidget):
        ...
        self.makeFitButton.setStatusTip(
            "Fit the profile with the current settings."
        )
    ...
    def createParametersInput(self, parentWidget):
        ...
        self.amplitudeEntry.setStatusTip(
            "Input for the amplitude of the profile."
        )
        self.amplitudeOutput = qtw.QLineEdit()
        self.amplitudeOutput.setEnabled(False)
        self.amplitudeOutput.setStatusTip(
            "Output for the amplitude of the profile."
        )
        ...
        self.centerEntry.setStatusTip(
            "Input for the center of the profile."
        )
        self.centerOutput = qtw.QLineEdit()
        self.centerOutput.setEnabled(False)
        self.centerOutput.setStatusTip(
            "Output for the center of the profile."
        )
        ...
        self.widthEntry.setStatusTip(
            "Input for the width of the profile."
        )
        self.widthOutput = qtw.QLineEdit()
        self.widthOutput.setEnabled(False)
        self.widthOutput.setStatusTip(
            "Output for the width of the profile."
        )
        ...
        self.offsetEntry.setStatusTip(
            "Input for the offset of the profile."
        )
        self.offsetOutput = qtw.QLineEdit()
        self.offsetOutput.setEnabled(False)
        self.offsetOutput.setStatusTip(
            "Output for the offset of the profile."
        )
    ...
    def createFitResults(self, parentWidget):
        ...
        self.contrastOutput.setStatusTip(
            "Contrast of the profile."
        )
        ...
        self.noiseOutput.setStatusTip(
            "Noise of the profile."
        )
        ...
        self.snrOutput.setStatusTip(
            "Signal-to-Noise Ratio of the profile."
        )
    ...
    def createResultsTable(self, parentWidget):
        ...
        self.resultsTable.setHorizontalHeaderLabels(['Name', 'Contrast', 'ContrastErr', 'Noise', 'NoiseErr', 'SNR', 'SNRErr', 'Frame', 'X', 'Y', 'Angle', 'Length', 'Fit', 'Amplitude', 'AErr', 'Center', 'CErr', 'Width', 'WErr', 'Offset', 'OErr'])
    ...
    def generateProfileLine(self, event=None, makeFit=False):
        ...
        p1, p2 = self.parent.imageTabsImage[tabIndex].drawNewProfile(self.currentPosition, self.currentAngle, self.currentLength)

        # Compute the profile and display it
        self.distance, self.profile = self.parent.imageTabsImage[tabIndex].extractProfile(p1, p2, self.currentPosition)

        # Fit profiles if required
        if makeFit or self.parent.controlPanel.liveFitCheckBox.isChecked():
            self.initialParameters = mfunc.initialiseParameters(self.distance, self.profile)
    ...
    def fitProfile(self):
        ...
        self.fitParameters, self.fitErrors = mfunc.fitProfile(self.distance, self.profile, initialParameters, fitType=self.fitType)
    ...
    def plotProfile(self):
        ...
        if self.prefitDisplayCheckBox.isChecked():
            self.profileAxis.plot(self.fittedDistance, mfunc.fittedProfile(self.fittedDistance, self.getInitialParameters(), fitType=self.returnFitType()), 'r--', linewidth=2)

        # Plot the plot
        self.fittedShortProfile = mfunc.fittedProfile(self.distance, self.fitParameters, fitType=self.returnFitType())
        self.fittedProfile = mfunc.fittedProfile(self.fittedDistance, self.fitParameters, fitType=self.returnFitType())
        self.profileAxis.plot(self.fittedDistance, self.fittedProfile, 'b-', linewidth=4)
    ...
    def returnFitType(self):
        ...
        buttonState = np.array([self.sincFunctionRadiobutton.isChecked(), self.gaussFunctionRadiobutton.isChecked()])
    ...
    def calculateContrastNoise(self):
        ...
        fittedSignal = self.distance, mfunc.fittedProfile(self.distance, self.fitParameters, fitType=self.returnFitType())
        imageValues, imageValuesErr = mfunc.computeSNR(signal, fittedSignal, self.fitParameters, self.fitErrors)
        self.currentContrast, self.currentNoise, self.currentSNR = imageValues
        self.currentContrastErr, self.currentNoiseErr, self.currentSNRErr = imageValuesErr

        # Update the display
        contrastText = str(round(self.currentContrast,3)) + ' ± ' + str(round(self.currentContrastErr,3))
        self.contrastOutput.setText( contrastText )
        noiseText = str(round(self.currentNoise,3)) + ' ± ' + str(round(self.currentNoiseErr,3))
        self.noiseOutput.setText( noiseText )
        snrText = str(round(self.currentSNR,3)) + ' ± ' + str(round(self.currentSNRErr,3))
    ...
    def storeProfile(self):
        ...
        dataName = self.parent.imageTabsImage[tabIndex].name + '_'+str(numberStoredData + 1)

        imageValues = [ round(self.currentContrast,3), round(self.currentContrastErr,3), round(self.currentNoise,3), round(self.currentNoiseErr,3), round(self.currentSNR,3), round(self.currentSNRErr,3) ]

        frameNumber = currentImage.currentFrame

        positionAndSetup = [ int(self.currentPosition[0]), int(self.currentPosition[1]), int(self.currentAngle), int(self.currentLength) ]
        ...
        currentImage.savedData.append( iScatSignal(dataName, imageValues, frameNumber, positionAndSetup, fitType, fitParams, fitErrors, signal, fittedSignal) )
    ...
    def tableIsDoubleClicked(self, row, column):
        ...
        msg.setInformativeText("""Do you want to delete this profile from the current memory?""")
        msg.setWindowTitle("")
